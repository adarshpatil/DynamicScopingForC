...
public:
  ...
  bool isNewVarName( std::string v ){
	  if (std::find(allVarNames.begin(), allVarNames.end(), v) != allVarNames.end())
		  return 0;
	  return 1;
  }
  ...
  bool VisitVarDecl(const VarDecl *D){
	  if ( isNewVarName( D->getName() ) )
		allVarNames.push_back( D->getName() );
	  
	  if ( D->isLocalVarDecl() )
		llvm::errs() << "LOCAL VAR " << D->getName() << "\n";
  }
  ...
  void dispVar(){
	  ...
	  for( std::vector<std::string	>::iterator it = allVarNames.begin(); it != allVarNames.end(); ++it){
		  llvm::errs() << *it << " ";
	  }
	  llvm::errs() << "\n";
  }
  void addGlobalDecl(){
	  std::stringstream SST;
	  SourceLocation GV = SourceLocation::getFromRawEncoding(declLoc);
	  for( std::vector<std::string	>::iterator it = allVarNames.begin(); it != allVarNames.end(); ++it){
		  SST << "DT " << *it << "; ";
		  SST << "DT." << *it << ".type = 0;\n";
	  }
	  SST << "\n\n";
	  TheRewriter.InsertText(GV, SST.str(), true, true);  
  }
private:
  Rewriter &TheRewriter;
  std::vector<std::string> allVarNames;
  unsigned int declLoc;
};
